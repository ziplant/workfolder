...
function cleanDist() {
	return src([`${distDir}/**/*.*`, `${distDir}/**/*`], {read: false})
		.pipe(clean())
}
...
function watchFiles() {
  watch('dev/static/**/*.sass', toCSS)
	watch('dev/components/**/*.sass', toComponentCSS)
	watch('dev/**/*.pug', toHTML)
	watch('dev/**/*.js', babelJS)
	watch(['dev/static/**/*', '!dev/static/sass/**', '!dev/static/js/**'], copyStatic)
